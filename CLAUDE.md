...
`cookie-domain.html` is the exception: arriving with an id already set is the
case it exists to show, so it clears on demand instead, and each of its probes
clears before it seeds.
